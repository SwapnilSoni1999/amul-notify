...
const USAGE = `${emojis.info} Usage: /freetrial 3d @username`
...
export const freeTrialCommand: MiddlewareFn<CommandContext> = async (
  ...
  const [durationArg, usernameArg] = args

  if (args.length !== 2 || !durationArg || !usernameArg) {
  ...
  if (!TELEGRAM_USERNAME_REGEX.test(username)) {
    await ctx.reply(`${emojis.warning} Invalid Telegram username. ${USAGE}`)
    return next()
  }
  ...
  if (!user) {
    await ctx.reply(`${emojis.crossMark} User @${username} was not found.`)
    return next()
  }
  ...
  await ctx.reply(
    [
      ...
      `User: <b>@${user.tgUsername || username}</b>`,
