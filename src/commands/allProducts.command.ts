...
export const allProductsCommand: MiddlewareFn<CommandContext> = async (
  ctx,
  next
) => {
  ...
  const products = await ctx.amul.getAllProducts({
    search: query
  })

  const title = `<b>All Amul Products</b> (${ctx.amul.getPincode()} - ${ctx.amul.getSubstore()})`
